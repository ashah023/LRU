...
int CLOCK_ALGO = 0;	// toggles the new clock algorithm
EXPORT_SYMBOL(CLOCK_ALGO);
				...
				struct zone *zone, struct scan_control *sc)
{
	...
	do {
		...
		// We get the total that was taken
		zone->nr_evicted += nr_taken; //Maylem & Renan
		...
		while (!list_empty(&page_list)) {
			...
			if (PageActive(page)){
				...
				// Remove those that were put back in.
				// This gives the total that were evicted.
				zone->nr_evicted--; //  Maylem & Renan
			}
...
force_reclaim_mapped:
	...
	while (!list_empty(&l_hold)) {
		...
		// Renan & Maylem
		// We check to see if we are going to use the Clock algo
		if(CLOCK_ALGO){
			//If there is not an overflow...
			if(page->frame_count + 1 > page->frame_count){
				// The page frame counter is incremented by
				// the value of the reference bit, and the
				// reference bit is then cleared
				page->frame_count += page_referenced(page, 0);
			}
			else{
		...
		if (page_mapped(page)) {
			if (!reclaim_mapped ||
			    ...
			    CLOCK_ALGO ? (page->frame_count > 0) : page_referenced(page, 0)) {
				// Above condition checks if we are to use the Clock algo.
				// If so, check to see if the reference counter is at least one.
				// This allows us to put the page to the back of the active list.
		...
		if(CLOCK_ALGO){
			// If the reference counter has reached zero, the page needs to
			// be evicted to the inactive list.
	...
	while (!list_empty(&l_inactive)) {
		...
		if (!pagevec_add(&pvec, page)) {
			...
			// This the number switched to inactive list
			zone->nr_swt_inactive += pgmoved; // Maylem & Renan
			spin_unlock_irq(&zone->lru_lock);
	...
	zone->nr_inactive += pgmoved;
	// This is the number switched to the inactive list
	zone->nr_swt_inactive += pgmoved;	// Maylem & Renan
	pgdeactivate += pgmoved;
	if (buffer_heads_over_limit) {
		spin_unlock_irq(&zone->lru_lock);
